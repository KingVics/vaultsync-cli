 ...
 */
function wrapKey(aesKey, machinePublicKeyPem) {
    return publicEncrypt({ key: machinePublicKeyPem, padding: constants.RSA_PKCS1_OAEP_PADDING, oaepHash: 'sha256' }, aesKey);
}
...
export const policyCmd = new Command('grant')
    ...
    try {
        ...
        const machineList = machinesResult.machines;
        const machine = machineList.find(m => m.name === opts.machine);
        if (!machine) {
            throw new Error(`Machine "${opts.machine}" not found.\n` +
